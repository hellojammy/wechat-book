...
class Wechat{
    ...
    const ACCESS_TOKEN_CACHE = 'access_token_cache';
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    protected function removeCache($cachename) {
        if($this->CI){
            $this->CI->cache->save($cachename, '' , 1);
        }
    }
    ...
    public function getJsSign($url = '', $timestamp = 0, $noncestr = '', $appid = '') {
        ...
        //remove pajax
        $url = str_replace("?_pjax=%23main", "", $url);
        $url = str_replace("?_pjax=#main", "", $url);
        $url = str_replace("&_pjax=%23main", "", $url);
        $url = str_replace("&_pjax=#main", "", $url);
        $url = trim($url);
        if (empty($url))
